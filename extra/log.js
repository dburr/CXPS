...
const output = function(locations, label, message){
  ...
  if (typeof message === "object"){
    if (message instanceof Error){
      output(locations, label, message.stack);
      return;
    }
    message = circularJSON.stringify(message, null, 4);
...
function create(level, label, options){
  ...
  if (typeof options === "object"){
    ...
    if (typeof options.colors === "object"){
      this.colors = extend(true, DEFAULT_COLORS, options.colors);
    }
  }
  ...
  this.always = function(message, _label){
    output(outputLocations, {text: _label || label, bgColor: this.colors.always.bg, color: this.colors.always.fg}, message);
  };
